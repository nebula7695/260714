...
# MBTI별 데이터 (포켓몬 공식 홈페이지의 안정적인 이미지 URL 사용)
mbti_data = {
    "ISTJ": {
        "job": "회계사 / 데이터 분석가",
        "pokemon": "메타그로스",
...
if selected_mbti:
    ...
    with col2:
        # 최신 스트림릿 버전에 맞춘 속성 사용 (use_container_width)
        st.image(info['img'], caption=f"{selected_mbti}와 어울리는 {info['pokemon']}", use_container_width=True)

st.divider()
st.caption("포켓몬 이미지 출처: 포켓몬스터 공식 홈페이지")
